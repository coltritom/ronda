...
export default async function DebesPage() {
  ...
  if (groupIds.length === 0) {
    return <EmptyState icon="✓" title="Sin deudas" sub="No estás en ningún grupo todavía." />
  }
  ...
  if (eventIds.length === 0) {
    return <EmptyState icon="✓" title="Sin deudas" sub="No hay juntadas en tus grupos todavía." />
  }
  ...
  if (rows.length === 0) {
    return <EmptyState icon="✓" title="Estás al día" sub="No debés nada en ningún grupo." />
  }

  const byGroup: Record<string, DebtRow[]> = {}
  for (const r of rows) {
