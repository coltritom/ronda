-- Fix upcoming/past event split to use Argentine date instead of UTC datetime.
-- Both home and group page now treat an event as "upcoming" for the entire
-- day it occurs (Argentine timezone), transitioning to "past" at midnight.
...
BEGIN
  ...
  last_ten AS (
    ...
    WHERE group_id = ANY(v_group_ids)
      ...
      AND (date AT TIME ZONE 'America/Argentina/Buenos_Aires')::date < (NOW() AT TIME ZONE 'America/Argentina/Buenos_Aires')::date
    ORDER BY date DESC LIMIT 10
  ),
  att AS (
    SELECT
      COUNT(*) FILTER (WHERE ea.user_id IS NOT NULL AND ea.attended = true)::int AS attended,
      COUNT(*)::int AS total
    FROM last_ten lt
    LEFT JOIN event_attendance ea ON ea.event_id = lt.id AND ea.user_id = p_user_id AND ea.attended = true
  ),
  ...
  b_att AS (
    ...
    WHERE ev.group_id = ANY(v_group_ids) AND ea.attended = true
